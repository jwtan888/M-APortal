...
(function(){
  ...
  function writePortalMetadata(wb,payload){let ws=wb.getWorksheet('_MA Portal Data');if(!ws)ws=wb.addWorksheet('_MA Portal Data');ws.state='veryHidden';ws.getCell('A1').value='M&A Portal Consumption';ws.getCell('A2').value='Season';ws.getCell('B2').value=clean(payload.season);ws.getCell('A3').value='Style';ws.getCell('B3').value=clean(payload.style);ws.getCell('A4').value='Proto Stage';ws.getCell('B4').value=clean(payload.protoStage);ws.getCell('A5').value='Format Version';ws.getCell('B5').value=1;}
  ...
  window.generateExcelBrowser=async function(payload,helpers){const wb=await loadTemplateWorkbook();const master=readMaster(wb);const rows=buildRows(payload.rows||[],master);if(!rows.length)throw new Error('No calculator rows were supplied.');writePortalMetadata(wb,payload);writeMaterialList(wb,master.materials);writeData(wb,rows);writeBreakdown(wb,rows);writeSummary(wb,rows);wb.calcProperties=wb.calcProperties||{};wb.calcProperties.fullCalcOnLoad=true;wb.calcProperties.forceFullCalc=true;wb.calcProperties.calcMode='auto';const buffer=await wb.xlsx.writeBuffer();helpers.downloadBlob(new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),filename(payload));return buffer;};
})();
